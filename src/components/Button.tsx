...
function Button({ text, type, block, light, className, onClick, loading, disabled, color }: buttonProps) {
  ...
  return (
    <div className={`${block ? 'block' : 'inline-block'}`}>
      <button type={type} onClick={onClick} disabled={loading || disabled} className={`${block ? 'block w-full' : 'inline-block'} ${btnColor()} px-2 py-0.5 outline-none transition duration-300 rounded disabled:cursor-not-allowed select-none ${className}`}>
